...
static void arch_map_pmm_data_space(paddr kernel_phy_start, paddr kernel_phy_end,
    ...
    for (; pmm_data_phy_start_addr < pmm_data_phy_end;
        ...
        pmm_data_start_round_down_2m = ROUND_DOWN(pmm_data_phy_start_addr, MIDDLE_PAGE_SIZE);
        arch_set_L2_entry_huge(
            pmm_data_start_round_down_2m, KERNEL_PHY_TO_VIRT(pmm_data_start_round_down_2m),
            (union L2_entry_huge *)&L2_table, (PT_DESC_V | PT_DESC_ATTR_LOWER_AF));
    }
}
...
void arch_init_pmm(struct setup_info *arch_setup_info) {
    ...
    pmm_data_phy_start =
        ROUND_UP(KERNEL_VIRT_TO_PHY(arch_setup_info->map_end_virt_addr), PAGE_SIZE);
    ...
    for (int i = 0; i < m_regions.region_count; i++) {
        ...
        if (kernel_phy_start >= reg->addr && map_end_phy_addr <= region_end) {
            ...
            // only one size is used, just change the region
                ...
                m_regions.memory_regions_insert(reg->addr, kernel_phy_start - reg->addr);
                reg->addr = map_end_phy_addr;
    ...
    arch_map_pmm_data_space(kernel_phy_start, kernel_phy_end, pmm_data_phy_start, pmm_data_phy_end);
    generate_pmm_data(kernel_phy_start, kernel_phy_end, pmm_data_phy_start, pmm_data_phy_end);
    return;
arch_init_pmm_error:
    arch_shutdown();
}
