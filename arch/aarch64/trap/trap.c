 ...
 *
 * | EC  | Name                          | Source          | trap_class           | Verified |
 * |-----|-------------------------------|-----------------|----------------------|----------|
 ...
 * | 0x0d| Alignment fault               | ARM ARM         | ALIGNMENT            | ✅       |
 * | 0x0e| Illegal execution state       | ARM ARM         | ILLEGAL_INSTR        | ✅       |
 * | 0x0f| SError from lower EL         | ARM ARM         | ASYNC_ABORT          | ✅       |
 * | 0x10| (not an EC value)            | -               | -                    | ❌       |
 * | 0x11| Tag check fault (MTE)        | ARM ARM         | SECURITY             | ✅       |
 * | 0x15| SVC from lower EL            | ARM ARM         | SYSCALL              | ✅       |
 * | 0x18| SVC in AArch64               | ARM ARM         | SYSCALL              | ✅       |
 * | 0x20| Instruction abort (lower EL) | ARM ARM         | PAGE_FAULT           | ✅       |
 * | 0x21| Instruction abort (same EL)  | ARM ARM         | PAGE_FAULT           | ✅       |
 * | 0x22| PC alignment fault           | ARM ARM         | ALIGNMENT            | ✅       |
 * | 0x24| Data abort (lower EL)        | ARM ARM         | PAGE_FAULT           | ✅       |
 * | 0x25| Data abort (same EL)        | ARM ARM         | PAGE_FAULT           | ✅       |
 * | 0x26| SP alignment fault           | ARM ARM         | ALIGNMENT            | ✅       |
 * | 0x2c| FP exception (AArch32)      | ARM ARM         | FP_FAULT             | ✅       |
 * | 0x2f| SError interrupt            | ARM ARM         | ASYNC_ABORT          | ✅       |
 * | 0x30| Breakpoint (lower EL)       | ARM ARM         | BREAKPOINT           | ✅       |
 * | 0x31| Breakpoint (same EL)        | ARM ARM         | BREAKPOINT           | ✅       |
 * | 0x32| Software step                | ARM ARM         | DEBUG                | ✅       |
 * | 0x33| Watchpoint                   | ARM ARM         | DEBUG                | ✅       |
 * | 0x34| Vector catch                 | ARM ARM         | DEBUG                | ✅       |
 * | 0x35| Other debug exceptions       | ARM ARM         | DEBUG                | ✅       |
...
static enum trap_class aarch64_ec_to_trap_class(u64 ec)
{
        ...
        case 0x00: return TRAP_CLASS_UNKNOWN;           /* Unknown reason */
        case 0x01: return TRAP_CLASS_UNKNOWN;           /* Trapped WFI/WFE */

        /* Trapped instruction execution */
        case 0x02: return TRAP_CLASS_ILLEGAL_INSTR;     /* Trapped MRS/MSR */
        case 0x03: return TRAP_CLASS_ILLEGAL_INSTR;     /* Trapped CP15 MRC/MCR */
        case 0x04: return TRAP_CLASS_ILLEGAL_INSTR;     /* Trapped CP14 MRRC/MCRR */
        case 0x05: return TRAP_CLASS_ILLEGAL_INSTR;     /* Trapped SVE/SIMD/FP */
        case 0x06: return TRAP_CLASS_ILLEGAL_INSTR;     /* Trapped other instructions */

        /* Floating-point and Advanced SIMD exceptions */
        case 0x07: return TRAP_CLASS_FP_FAULT;          /* FP access trap */
        case 0x08: return TRAP_CLASS_FP_FAULT;          /* FP exception */
        case 0x2c: return TRAP_CLASS_FP_FAULT;          /* FP exception (AArch32) */

        /* Memory access faults */
        case 0x0d: return TRAP_CLASS_ALIGNMENT;         /* Alignment fault */
        case 0x0e: return TRAP_CLASS_ILLEGAL_INSTR;     /* Illegal execution state */
        case 0x11: return TRAP_CLASS_SECURITY;          /* Tag check fault (MTE) */

        /* Instruction aborts (page faults) */
        case 0x20: return TRAP_CLASS_PAGE_FAULT;        /* Instruction abort from lower EL */
        case 0x21: return TRAP_CLASS_PAGE_FAULT;        /* Instruction abort from same EL */
        case 0x22: return TRAP_CLASS_ALIGNMENT;         /* PC alignment fault */

        /* Data aborts (page faults) */
        case 0x24: return TRAP_CLASS_PAGE_FAULT;        /* Data abort from lower EL */
        case 0x25: return TRAP_CLASS_PAGE_FAULT;        /* Data abort from same EL */
        case 0x26: return TRAP_CLASS_ALIGNMENT;         /* SP alignment fault */

        /* System-related exceptions */
        case 0x15: return TRAP_CLASS_SYSCALL;           /* SVC from lower EL (AArch64) - user->kernel syscall */
        case 0x18: return TRAP_CLASS_SYSCALL;           /* SVC in AArch64 - same-EL SVC (defensive) */
        /*
         * Note: We map both EC 0x15 and 0x18 to SYSCALL for completeness:
         * - EC 0x15: Normal syscall path (SVC from EL0 to EL1)
         * - EC 0x18: Same-EL SVC (should not happen in normal operation, but we handle it defensively)
         *
         * Most syscalls will use EC 0x15. EC 0x18 is a defensive catch for
         * erroneous same-EL SVC instructions (kernel bugs, compromised kernel, etc.).
         */

        /* Debug exceptions */
        case 0x30: return TRAP_CLASS_BREAKPOINT;        /* Software breakpoint from lower EL */
        case 0x31: return TRAP_CLASS_BREAKPOINT;        /* Software breakpoint from same EL */
        case 0x32: return TRAP_CLASS_DEBUG;             /* Software step */
        case 0x33: return TRAP_CLASS_DEBUG;             /* Watchpoint */
        case 0x34: return TRAP_CLASS_DEBUG;             /* Vector catch */
        case 0x35: return TRAP_CLASS_DEBUG;             /* Other debug exceptions */
        case 0x3c: return TRAP_CLASS_BREAKPOINT;        /* Software step (BKPT from AArch32) */

        /* Asynchronous errors */
        case 0x0f: return TRAP_CLASS_ASYNC_ABORT;       /* SError from lower EL */
        case 0x2f: return TRAP_CLASS_ASYNC_ABORT;       /* SError interrupt */

        /* Uncategorized/Reserved EC values */
        default: return TRAP_CLASS_UNKNOWN;
        }
}
...
void arch_populate_trap_info(struct trap_frame *tf, struct aarch64_trap_info *info)
{
        ...
        case TRAP_CLASS_PAGE_FAULT: {
                ...
                if (!is_inst_abort) {
                        ...
                        info->esr_fields.wnR = (info->esr_fields.iss >> 6) & 0x1;
                         ...
                         *
                         * Permission faults (page present, but insufficient permission):
                         ...
                         *
                         * is_present logic:
                         *   - Translation faults → is_present = false (page not present)
                         *   - Access flag/Permission faults → is_present = true (page exists)
                         */
                        u8 dfsc = info->esr_fields.dfsc;
                        info->is_present = !((dfsc >= 0x04 && dfsc <= 0x07) || dfsc == 0x2B);
                } else {
                        /* Instruction abort: parse IFSC */
                        info->esr_fields.ifsc = info->esr_fields.iss & 0x3F;

                        /*
                         * IFSC (Instruction Fault Status Code) uses same encoding as DFSC.
                         * Translation faults (0x04-0x07, 0x2B) → page not present.
                         * Other faults (access flag, permission) → page present.
                         */
                        u8 ifsc = info->esr_fields.ifsc;
                        info->is_present = !((ifsc >= 0x04 && ifsc <= 0x07) || ifsc == 0x2B);
                }
                        ...
                        u64 irq_attr)
{
        ...
        for (u64 ec = 0; ec < 0x40; ec++) {
                if (aarch64_ec_to_trap_class(ec) == trap_class) {
                        register_irq_handler(ec, aarch64_fixed_trap_wrapper, irq_attr);
                }
        }
}
