		...
		unsigned long arg4, unsigned long arg5)
{
	...
	__asm__ __volatile__("ecall" : "+r"(a0), "+r"(a1) : "r"(a2), "r"(a3),
...
{
	return (sbi_ecall(SBI_DBCN_EXT, SBI_EXT_DBCN_CONSOLE_WRITE, 0, 0, 0, 0, 0,
			0));
}
struct sbiret	sbi_debug_console_write_byte(u_int8_t byte)
{
	return (sbi_ecall(SBI_DBCN_EXT, SBI_EXT_DBCN_CONSOLE_WRITE_BYTE, 0, 0, 0, 0,
			0, 0));
}
...
{
	return (sbi_ecall(SBI_SRST_EXT, SBI_EXT_SRST_RESET,
			(unsigned long)reset_type, (unsigned long)reset_reason, 0, 0, 0,
			0));
}
