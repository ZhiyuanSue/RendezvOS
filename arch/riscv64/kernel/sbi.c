...
struct sbiret sbi_ecall(int eid, int fid, unsigned long arg0, unsigned long arg1,
...
struct sbiret sbi_system_reset(u_int32_t reset_type, u_int32_t reset_reason) {
    return (sbi_ecall(SBI_SRST_EXT, SBI_EXT_SRST_RESET, (unsigned long)reset_type,
                      (unsigned long)reset_reason, 0, 0, 0, 0));
}
