...
error_t	enable_IRQ(int irq_num)
{
	...
	if (irq_num >= _8259A_MASTER_IRQ_NUM_ && irq_num < _8259A_MASTER_IRQ_NUM_
		+ _8259A_IRQ_NUM_)
	{ /*master irq*/
		_8259A_IMR_MASTER_ = clear_bit(_8259A_IMR_MASTER_, (irq_num
					- _8259A_MASTER_IRQ_NUM_));
		outb(_X86_8259A_MASTER_1_, _8259A_IMR_MASTER_);
	}
	else if (irq_num >= _8259A_SLAVE_IRQ_NUM_ && irq_num < _8259A_SLAVE_IRQ_NUM_
		+ _8259A_IRQ_NUM_)
	{ /*slave irq, need enable the irq
			2 at master, too*/
		_8259A_IMR_SLAVE_ = clear_bit(_8259A_IMR_SLAVE_, (irq_num
					- _8259A_SLAVE_IRQ_NUM_));
...
error_t	disable_IRQ(int irq_num)
{
	if (irq_num >= _8259A_MASTER_IRQ_NUM_ && irq_num < _8259A_MASTER_IRQ_NUM_
		+ _8259A_IRQ_NUM_)
	{ /*master irq*/
		_8259A_IMR_MASTER_ = set_bit(_8259A_IMR_MASTER_, (irq_num
					- _8259A_MASTER_IRQ_NUM_));
		outb(_X86_8259A_MASTER_1_, _8259A_IMR_MASTER_);
	}
	else if (irq_num >= _8259A_SLAVE_IRQ_NUM_ && irq_num < _8259A_SLAVE_IRQ_NUM_
		+ _8259A_IRQ_NUM_)
	{ /*slave irq*/
		_8259A_IMR_SLAVE_ = set_bit(_8259A_IMR_SLAVE_, (irq_num
					- _8259A_SLAVE_IRQ_NUM_));
		outb(_X86_8259A_SLAVE_1_, _8259A_IMR_SLAVE_);
	}
	else
