...
static error_t arch_get_memory_regions(struct setup_info *arch_setup_info) {
    ...
    for (mmap = (struct multiboot_mmap_entry *)add_ptr; ((vaddr)mmap) < (add_ptr + length);
         mmap = (struct multiboot_mmap_entry *)((vaddr)mmap + mmap->size + sizeof(mmap->size))) {
...
static void arch_map_pmm_data_space(paddr kernel_phy_start, paddr kernel_phy_end,
    ...
    for (; pmm_data_phy_start_addr < pmm_data_phy_end;
        ...
        arch_set_L2_entry_huge(pmm_data_start_round_down_2m,
                               ...
                               (union L2_entry_huge *)&L2_table, (PDE_P | PDE_RW | PDE_G | PDE_PS));
    }
}
