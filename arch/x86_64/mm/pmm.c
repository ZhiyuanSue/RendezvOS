...
static void arch_map_pmm_data_space(paddr kernel_phy_start,
	...
	paddr pmm_data_phy_start_addr = pmm_data_phy_start;
...
void arch_init_pmm(struct setup_info *arch_setup_info) {
	...
	paddr pmm_data_phy_start = ROUND_UP(kernel_phy_end, PAGE_SIZE);
	paddr pmm_data_phy_end = 0;
	int kernel_region = -1;
	...
	for (int i = 0; i < m_regions.region_count; i++) {
		...
		if (kernel_phy_start >= reg->addr && kernel_phy_end <= region_end) {
			...
			// only one size is used, just change the region
				...
				m_regions.memory_regions_insert(reg->addr,
