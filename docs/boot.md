...
1、最开始，需要关闭中断cli（直到配置好中断向量之前都不应该开）
2、为了支持函数调用，所以需要把multiboot传递进来的寄存器参数进行保存。
3、其次，设置好堆栈指针
4、分页必须在保护模式下启用，CR0.PE = 1，这一步已经由multiboot所保证
5、但是multiboot下CR0. PG = 0，因此需要先开启这一步。
6、随后开启CR4.PAE = 1，使用PAE分页模式
7、之后设置IA32_EFER.LME = 1 ，开启四级分页，也可以继续设置 CR4.LA57 = 1，开启5级分页
