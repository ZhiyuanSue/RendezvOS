...
不过，上述的方案并不完善，实际上，这个tagged ptr可能用于多个地方，还是会丢失信息。
更简单的做法，那就是，在所有需要message的地方，都额外加一条allocator id的信息。

当然，即便如此，锁还是要加的，因为它可能试图释放其他核心上的内容，但区别在于，一个普通的cas锁即可。因为，通常来说，我们认为，每个核心在各自的内存分配器或者页分配器上搞事情，但很少会试图调用别的核心上的，这和全局只有一个物理页分配器是不同的。

最后，注意区分用户态和内核态。首先nexus的目前设计是一棵树，每个节点意味着一个vspace，每个vspace下再有一棵树用来管理页面，根为内核页面。而spmalloc是内核对象分配器，只会在vspace的根树下的内核页面中分配。这不会发生迁移，但是用户态的vspace会存在迁移。因此，它无需考虑什么迁移之后带来的spmalloc申请的页面在另一个core的内容上。只需要考虑锁住其他core，然后释放（当然也需要锁alloc）。
...
当先一个，就是port和进程的关系问题，也就是，port的数据结构怎么组织的

首先全局都应该只有一个用于记录port信息的数据结构，然后每个核心都应该有一个thread用于ipc server，我认为读写锁是必要的，因为这种发送消息大部分情况都应该是读出来，但是少部分的情况是写入这个port信息。

因此呢，就有读和写两种类型的消息发送。

其次是端口和进程的关系问题，参考sel4的实现，每个port之后都应该有一个tcb队列，当然，我认同使用无锁队列即可，因为sel4的实现也是一个head加tail的数据结构。

从这个角度来说，endpoint只是找到对应的等在相应端口的进程。而消息则是在发送的时候挂在找到的对应的进程控制块上面的消息队列上。（请务必区分这两个过程）

所以实际上，还需要在tcb这里维护好一个进程消息队列（也可能是多个）

#### 同步和异步IPC问题

由于同步和异步的关系问题，这必然就没法子跟tcb这部分解耦合了。当然其实我也没指望这部分解耦合。

同步的话，发送方在对应的endpoint这里，找到相应的线程控制块，把消息挂在对应的线程的消息队列上，如果找不到，就把原先的进程设为block on send

找到之后，按我的理解，就可以直接切换到对应线程的消息处理流程上去了。

异步IPC,自然是直接发，不过，关于消息怎么办，我理解它可以在endpoint这里增加一个消息缓冲队列，也就是，哪怕没有消息接收者，也能发出去，当然这直接返回了，并不会直接切换。如果有接收者，才考虑直接切换过去。

同样的对于接收者来说，如果只有同步，那么没有人发，会把自己挂队列头上。

但是加上异步就是，先查看有没有人发，其次查看是否有缓存的消息，都没有才自挂东南枝。

#### 无锁实现
我已经实现了一个无锁队列的基础结构，需要改写每个endpoint包含了，一个无锁的tcb队列，一个无锁的缓存消息的队列。

#### 批处理消息发送

TODO,按照我的理解，确实可以做一个用于多次发送的，比如高性能网卡这种发包的
但是慢慢来吧

#### 测试
这个还是好说的，仿照ms queue的测试方案，但是改成了不停从一个确定的endpoint获取内容。

======================

# 实际实现（上面都是设计，实际实现中碰到了更多的问题和逐步的思考）
## 整体框架

endpoint现在被命名为port，他的主要作用是匹配send和recv进程。

从设计上而言我实现了一个单队列的方案，这是因为，排在这个port上面的进程，要么全都是send进程要么全都是recv进程，否则一定会直接发生匹配。从而匹配剩下的要么都是send要么都是recv，要么没有，这导致了队列实际上还有一个状态，empty、send、recv三种状态。

send进程试图发送，会试图在队列中弹出一个recv进程，如果找不到，就压入队列，队列的状态现在就是send，recv相反。

在匹配之后，就会涉及到另一个事情，就是消息如何在send和recv进程之间进行传递的问题。我设计了一个message节点，在send之前创建，如果是send匹配上了一个recv，就从send线程移动到recv线程

第三个事情是，同步异步的语义和调度，这个得放到一起说，因为确实同步异步会影响执行流的阻塞还是直接循环等待或者其他方式，而且前面设计的时候，思考到底在哪个核心上运行也很值得一说。（TODO）

## 无锁队列
实际上，我之前实现了一个经典的ms queue无锁队列。遇到的问题挺多。

首先是实现了一个基本的tagged ptr。高16位用于tag，低48位用于ptr。这是一个基本的用于避免ABA问题的框架。（相关资料众多，我不废话了）

然后问题在于上面的队列状态问题，为了保证这种状态的原子性，我把状态也缝合在tagged中，（也就是ms queue操作里面的append info，用于存储状态，为了更加通用，我设置了这个info的长度可以配置，但是不能超过16位）

具体来说，如果状态有2位（empty、send、recv三种状态占用2位），那么高14位用于tagged ptr的aba，2位用于状态，最低48位照旧。

相应的，我额外增加了接口和设计，分别是队头弹出状态符合某个值的节点（这是先判断在弹出，保证了判断和弹出的原子性，如果弹出再判断，如果不符合，那原先那个节点还没法子处理了）的接口，以及队尾，在队尾节点符合某个值的时候才试图压入，从而保证在队列发生状态变化的竞态边界情况下，仍然可以正常工作。另外，在代码中，也包括了一些特殊的处理，比如empty状态的转换，头尾tagged ptr的append info的更新，如果你想改这些接口，请务必仔细阅读语义。

在此基础上我封装了ipc_port_try_match和ipc_port_enqueue_wait两个原语。

## 边界情况

额外的，考虑这样一个情况：线程还在队列中排队，但是线程要被清理掉了。这是可能发生的。

如果不加以区分，队列中还有指针，但是结构体已经被清理掉了，就很麻烦。

如果要在一个无锁队列中删除中间一个节点，就更麻烦（我认为可以实现，但是我懒得去这么整了）

### 分析

首先，第一个问题是，线程什么时候是死掉的。

在试图将自己入队的时候，他并不会死掉，因为我们做的是内核线程的设计，而这是封装好的原语之一，绝不会是调度点（和用户态用中断来搞调度不同），所以他最起码入队的时候，他正在让自己入队，内核没道理去清理掉一个正在运行的线程。

在试图出队一个匹配线程的时候，他可能搞到一个已经死掉的线程，但是仍然排队在队列中（因为无锁队列确实不好清理）

所以关键是，线程可以死，这没问题，但是线程控制块本身不能随着线程死掉直接释放。

对此我的办法如下：

在入队的时候，增加自己的线程控制块的ref count，并port ptr指向对应的port，因为语义是，线程和port会同时试图获取这个线程控制块的所有权，所以有两个ref count。

当我试图找到匹配线程的时候，如果匹配线程的控制块中的字段表明他还活着，匹配线程控制块的所有权从port转到了我手里，这不会减少ref count，但是port置空，如果匹配线程控制块的状态字段说明他死了，那么就减少ref count，对于线程的清理，他只是试图减少这个控制块的ref count。

同样的，当我跟匹配线程控制块完成了通信，我需要减少对该线程控制块的ref count

当线程控制块的refcount==0的时候就清理，谁把他弄到0，就谁清理。
