...
我们对此之后会做很多的改造。具体改造的结果可以看第十章列举的代码实现情况。
...
## 8.1 cancel ipc — 我们的方案中已经拒绝

这个问题完全是由前面提到的ipc request的循环引用计数问题产生的。

如果线程控制块不能拥有ipc request的指针。我如何才能告知，这些request已经被cancel了呢？

至于实现。如果队列中有多个Ipc_Request_t，我难以定位对应的线程控制块，这非常麻烦。

如果队列中只有一个Ipc_Request_t，这也是在阻塞时候的通常情形，我们的队列虽然设计为可以支持多个消息，但是阻塞的时候，实际上，要么试图发送一个，要么试图收到一个，没有成功的时候都阻塞，不会有下一个。如果要cancel它的发送，那么随便收一个并直接丢弃消息就行了。如果cancel它的接受，发送一个约定好的cancel消息，就行了。所以，专门实现一个cencel ipc，在我们的代码的实现下，看上去必须拒绝实现它。
...
这里遇到了一些问题，首先是我们的代码模型，模型是这样的，我们系统中的很多处理都是在这里以无锁ipc的消息的方式去实现的。那具体到中断的处理上，因为整体是一个分离式的内核方案，他在这里只应该实现机制。

我们确实提供了一堆中断注册接口，但是正如Linux有中断上半部和下半部的区分一样，在我们的模型中，他所有的都是ipc，所以逻辑上，应该是，接到一个中断之后，去发消息，这是很好理解的。

## 9.1 发送方设定问题

那问题来了，发送方是哪个thread呢？

它实际上是以rendezvOS本身而非任何一个线程的身份在试图发消息，如果直接用current thread去做，就会面临一系列问题，你没法保证current thread本身的发送队列是空的，而这里的发送又是找到目标线程之后，直接从队列头找到一个消息。那试图发送的中断消息是往尾部插（没有任何在这个无锁队列中进行头插的办法），试图发的中断消息就可能不一定是最后取出来的消息，所以这种情况会发生错误。

因此我们在发送方设定这里，需要拥有一个代行身份的线程，显然idle是完美的，它理论上本不应该发送和接受消息，所以它可以作为内核中断本身发送消息的代行线程。

## 9.2 发送队列和send pending msg指针的处理

最开始，考虑过直接用发送队列的方案（跟正常普通ipc一致），但是后来我意识到，这不符合现实且更复杂。

现实情况是，一次我们只能处理一次中断，嵌套中断是否可能存在呢？可能，但是我理解，正常情况下，嵌套的中断不应该再触发发送了。

既然每次只有一个需要发送的消息，我理解，使用队列是毫无必要的。

同时我们需要考虑发送失败的情况，所以原本就有的发送方的send pending msg指针就很好，我们直接用atomic的exchage的方式，塞入消息，然后在ipc transfer message里面自动的就能进行处理。这个指针原本是用于发送失败的时候暂存一个pending消息，并供下一次尝试的。但是在这里，我们直接写入它，也能取得相同的效果。

我们应当使用try send clean的模式，而非阻塞式的发送方式，我理解，中断消息丢失本身都是常见的事情。但是如果使用阻塞的方式，因为他在中断处理的过程中，就可能产生灾难性的后果，整个系统都可能直接玩不转了。而如果try失败，自然需要clean，当然clean也容易，我们直接再用NULL和send pending msg指针进行exchange，然后清理掉我们需要处理的消息。

## 9.3 两种不同发送模型

但是落实到代码中，它存在两种不同的模型。

第一种是类似于时钟中断这样的模式，不知道哪个线程，阻塞在一个port上，试图等待一个时钟中断发来的消息去唤醒他，这往往会需要手动的塞入一个定时器，从而主动说明自己阻塞在这里。

第二种是类似于网卡中断的模式，我们会有一个专门的网卡驱动线程，他可能同时处理发包和收包的问题，如果此时正在处理发包的代码，那自然不会等在某个port上，此时，我try send会因为对面没有port等在这边失败，最后就是丢包了。所以一个很直观的解决方式是，我们不再每次都从port中拿了，而是从port中拿到了对面的接受者，或者在初始化的时候，直接获取到接受线程，然后如8.3这样，收到消息之后就批量发送。

当然这两种模式都需要使用代行线程，且前面说的点也都依然有效。

## 9.4 接口选择说明

其实只说接口实现，我认为完全可以对send msg 接口做一定的改写，因为我们之所以会产生前面的这个系统发送的问题，就是因为，在send msg里面，我们默认的发送方就是current thread，如果可以直接改写它成为允许接口设定的任意thread，那么理论上可以允许调用者把自己以任意的发送者身份向其他线程发消息，但是这是非常危险的。

而我们这里虽然用一个系统thread代为发送，但是它是固定的，即使是特例也是固定的，我不认为我们应该轻易允许任意的操作接口。

# 十、代码实现情况及相关测试代码说明

## 10.1 测试代码链接及相关文件与上述内容的对应

我的测试代码现在已经在rendezvOS上跑通了：
