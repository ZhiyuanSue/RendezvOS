...
有了无锁队列之后，我试图改变一下，也就是说增加一个buffer队列，里面包含了需要目标核心的分配器释放的内容，如果其他核心的分配器试图释放这个，只是把它挂载到目标核心的分配器的buffer无锁队列上，目标核心在每次释放的时候，在除了本身的这次释放，还要查看一下这个buffer无锁队列，通过这种方式，不用锁定在chunk中的内容，当然因为页面的分配本身用一个红黑树实现，而我没有实现红黑树的无锁（好像也很困难），因此这部分依然需要加上锁，当然了，spmalloc作为一个内核分配器，本身他的整页整页的分配情况也不算多，所以算不上麻烦。
...
## 修改spmalloc
在之前的设计上，当超过一定页数之后，是交给nexus去分配的，但是这些元数据的记录依托于nexus的系统去记录他到底要了多少页。但是对于nexus系统来说，他不应当去记录上层分配了多少页，而是应当又spmalloc系统去记录，因此需要在spmalloc中额外增加一个红黑树用于记录这些信息。目前是释放0（原有的语义和现有的语义不匹配），所以导致了，实际上一直没释放，直接内存漏完了（笑死）。
