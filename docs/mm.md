...
有了无锁队列之后，我试图改变一下，也就是说增加一个buffer队列，里面包含了需要目标核心的分配器释放的内容，如果其他核心的分配器试图释放这个，只是把它挂载到目标核心的分配器的buffer无锁队列上，目标核心在每次释放的时候，在除了本身的这次释放，还要查看一下这个buffer无锁队列，通过这种方式，不用锁定在chunk中的内容，当然因为页面的分配本身用一个红黑树实现，而我没有实现红黑树的无锁（好像也很困难），因此这部分依然需要加上锁，当然了，kmalloc作为一个内核分配器，本身他的整页整页的分配情况也不算多，所以算不上麻烦。

为此，增加了allocator id字段和msq_node字段，同时，这也导致了需要重新计算，结果是
40/48/56/64/80/96/128/160/288/544/1056/2048

为此，也需要调整整个padding的情况
...
基于nexus，再实现了一个kmalloc的一个内核对象分配器。
...
对于内核而言，多核问题并不是问题。理由很简单，会在内核调用alloc pages的地方可能很多，而这个分配，一定是对应的每个cpu去单独分配的，这点没有问题，也就是说，这时候，每个cpu占用各自的一部分的页面。（由于他们之间使用锁去抢页帧分配器，所以一定是可以的。）而有问题的是什么呢，是在释放的时候，另一个cpu使用他的nexus去释放属于我的页面，这肯定会出问题——找不到。可是，在内核中会调用的这个free_pages函数的地方在哪里呢，一个是kmalloc的地方，但是kmalloc本身处理了多核的问题（设计为多个分配器如果释放的时候发现不是自己的，需要调用别人的对应的nexus去释放。）所以这没问题。
另外就是内核直接试图分配一批页面，随后释放的地方了。但是内核理论上不应该在这里进行分配若干个页面这种事情（除非使用kmalloc），因为这种数据结构确实太大了。我们可以认为，内核要么不使用超过2048大小的内存块并使用kmalloc管理，要么使用若干个页面大小的内容，并永久不释放，要么使用并且试图释放，并且额外记住他的nexus_id，并如果不是当前核心的nexus，需要加锁。
但是对于用户态，这件事情就不一样了。

kmalloc并不用于用户态，用户态也不应当使用kmalloc，用户态只能使用page分配器nexus
...
还有一个巨大的问题是，为什么要在nexus里维护这个映射关系，因为这个映射关系已经在页表中

但是如果直接从页表中查找，会面临另一个问题，那就是，它可能有很大的tlb flush开销。因此，添加这样一个映射关系细致地进行维护本身并无问题。
...
## 修改kmalloc
在之前的设计上，当超过一定页数之后，是交给nexus去分配的，但是这些元数据的记录依托于nexus的系统去记录他到底要了多少页。但是对于nexus系统来说，他不应当去记录上层分配了多少页，而是应当又spmalloc系统去记录，因此需要在kmalloc中额外增加一个红黑树用于记录这些信息。目前是释放0（原有的语义和现有的语义不匹配），所以导致了，实际上一直没释放，直接内存漏完了（笑死）。

## 改变参数顺序
具体来说，现有的接口参数顺序比较不一致。

## 更改物理内存管理结构
目前的思路是和buddy模块耦合严重的，在buddy里面建立了一个zone，其实不应该是这样，而是应该先划分zone（可能还有多个node？当然numa的事情将来再说吧），再在每个zone里面构造内存分配器。
也就是说，从层级关系而言，应该先有针对每个可用物理内存的这个page frame的管理，然后在上层包含了一个物理zone管理器的内容。最后再构造额外的内容用于存放所有静态物理页面的内存管理器。
而类似于反向页表的指针的内容，会在底层的page frame这里使用。
需要考虑的是，每个zone中也可能存在多个不连续的物理内存区间构成的sections。

同样的改写思路，现在的问题是，要确定一个逐步改写的方案，而不是一口气吃成胖子。

最核心的改造来自于数据结构的改变。

从下到上说，最底下应该是对应于每个物理page的表示，这应当被实现为一个数组的形式。考虑到每个zone中的section的情况。
所以最好是section后面加上一个page frame的柔性数组成员。

每个zone，通过一个双向链表，将这些个section串起来。但是这些zone本身可能只能静态去分配了。

锁的问题。原本所有的pmm都用一把大锁锁死。现在应该期望使用每个zone的一个mcs锁。（当然在改造初期没必要这样）

回到基本的内容，初始化的时候计算物理内存占用这件事情上，需要计算section加上page frame的空间，再加上每个zone的分配器的管理结构的内存占用。