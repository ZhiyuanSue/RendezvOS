...
| Arch | 注册方式 | 实现位置（典型） |
|------|----------|------------------|
| x86_64 | `MSR_IA32_LSTAR` ← `arch_set_syscall_entry()`；入口汇编保存 `trap_frame` 后 `sti` + `call syscall` | `arch/x86_64/boot/start_arch.c`, `trap/kernel_entry.S` |
| aarch64 | `register_fixed_trap(TRAP_CLASS_SYSCALL, arch_syscall_helper, …)`；helper 做 EL0 DAIF inherit 后 `call syscall` | `arch/aarch64/boot/start_arch.c`, `trap/trap.c` |
...
- Boot 通过 **`register_fixed_trap(TRAP_CLASS_SYSCALL, arch_syscall_helper, …)`** 注册；`register_fixed_trap()` 会扫描 `aarch64_ec_to_trap_class()`，为 **EC 0x15**（EL0 SVC）与 **EC 0x18**（同 EL SVC，防御）绑定 `aarch64_fixed_trap_wrapper` → `arch_syscall_helper`。
- **`arch_syscall_helper`**（`boot/start_arch.c`，static）：仅 **EL0** trap 将 user `SPSR` 的 I 位合并进 live DAIF（对齐 x86 `arch_enter_kernel` 的 `sti` 意图），再调用 portable **`syscall()`**；**不要**在 `linux_layer/syscall` 里做 DAIF 继承。
- EL0 **IRQ** 路径不做 DAIF inherit；返回用户仍靠 `trap_frame->SPSR` + `eret`。
