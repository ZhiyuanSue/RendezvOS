...
struct acpi_table_rsdp_2 {
	...
	u32 length; /* The length of the table, in bytes, including the header,
				   starting from offset 0. This field is used to record the size
				   of the entire table. This field is not available in the ACPI
				   version 1.0 RSDP Structure. */
	u64 xsdt_address;	  /* 64 bit physical address of the XSDT. */
	u8 extended_checksum; /* This is a checksum of the entire table, including
 ...
 */
struct acpi_gas {
	u8 address_space_id; /* The address space where the data structure or
							register exists. */
...
#define ACPI_GAS_ID_Functional_Fixed_HW 0x7F /* Functional Fixed Hardware */
	...
	u8 register_bit_width;	/* The size in bits of the given register.
								When addressing a data structure, this field must
							   be zero. */
	u8 register_bit_offset; /* The bit offset of the given register at the given
							   address. When addressing a data structure, this
							   field must be zero. */
...
#define ACPI_GAS_ACCESS_SIZE_Qword 4
	u64 address; /* The 64-bit address of the data structure or register in the
					given address space (relative to the processor). */
};
/*
