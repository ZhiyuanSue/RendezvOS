 ...
 *    story (unmap prefix, radix DELETE / restore flags, relink rmap, etc.) **or**
 *    a multi-segment walk where a later step failure must undo earlier steps.
 * -# **Callers should not re-encode** map/radix ordering and rollback at every
 *    call site. **L0 (@ref vmm_radix_tree_lock_range_big) is orchestrator-owned**
 *    (see @par Radix / L0 vs L2); this file owns L2-only acquire/release via
 ...
 * vmm_radix_tree_unlock_range_small); they never acquire or release L0. When the
 * orchestrator finishes its pass it unlocks L0. L0 serializes who may enter the
 * tree and supports interval discovery; L2 covers radix+map work on one range.
 *
 ...
 * count (@ref vmm_radix_tree_calculate_end_check). The caller may release L0 only
 * after the util returns (and may run an unmap loop between L2 critical sections,
 * e.g. @ref mm_user_utils_clean_range_and_unfill).
 *
 ...
 *   @p page_count pages are mapped; zero-fill runs after L2 unlock. If the radix
 *   INSERT lock sees **any** already-insertable overlap in that range, the whole
 *   call **fails** (no overwrite, no partial range). PMM alloc before lock
 *   failure is rolled back.
 * - @ref mm_user_utils_fill_page_with_exist_range: exactly **one** page; radix leaf must already
 *   exist and be LAZY without VALID. Otherwise **fails** — no implicit grow.
 ...
 *   already be wired (radix @c VALID + L3 @c have_mapped), **same** radix shadow
 *   flags and **same** PTE flags on every page (uniform range, typically from
...
typedef enum {
        /** @c set_mask is the new stored flags (after @c entry_flags_rm_sw_flags). */
 ...
 */
vaddr mm_user_utils_set_range_and_fill(struct VSpace* vs,
                                       vaddr range_start,
                                       size_t page_count,
                                       ENTRY_FLAGS_t flags);
 ...
 */
error_t mm_user_utils_fill_page_with_exist_range(struct VSpace* vs, vaddr page_va,
 ...
 */
error_t mm_user_utils_remap_page(struct VSpace* vs,
                                 vaddr page_va,
                                 ppn_t new_ppn,
...
/**
 * @brief Update PTE (and radix unless @c MM_USER_RANGE_FLAGS_DELTA_PTE_ONLY) on a
 *        contiguous range with **identical** per-page flags beforehand.
 *
 * **Pre:** caller holds big for the whole range (usually after @ref
 * vmm_radix_tree_find_first_occupied_interval).
 */
error_t mm_user_utils_set_range_flags(struct VSpace* vs,
                                      vaddr range_start,
                                      u64 length_bytes,
