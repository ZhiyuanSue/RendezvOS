 ...
 * ---------------------------------------------------------------------------
 * 1) `vmm_radix_tree_init` (+ optional `bootstrap` / `install_shared_kernel_high_half`
 *    for shared high-half L1). `handler` wires radix table pages at KERNEL linear.
 * 2) `insert_range` on [page_vaddr, page_vaddr + n*PAGE_SIZE): grows path if needed
 *    (RADIX_RL_INSERT), reserves every crossed leaf as LAZY and sets `vs_ptr` to
 *    `vs` for L0 index < 256, else to `root_vspace` for the shared high half
 *    (L0 index >= 256). Caller does not need PTEs yet.
 * 3) `leaf_bind_range` (or `leaf_bind` for one page): one `radix_range_lock_acquire`
 *    over the same VA span (RADIX_RL_QUERY_OR_CHANGE). Caller must have mapped
 *    PTEs; PPNs must be **contiguous** (`ppn_first`, `ppn_first+1`, …). Each leaf
 *    must be LAZY and **not** VALID; **do not** require `vs_ptr == vs` (shared L3
 *    may differ). Bind sets VALID + rmap only — **does not** rewrite `vs_ptr`
 *    (insert already fixed ownership). Overwrite/remap is `change_*`, not bind.
 *    Same `leaf_flags` apply to all pages; rollback restores lazy shadow from
 *    `leaf_flags` — match the prior `insert_range` flags on that band.
 * 4) `change_leaf_ppn` / `change_range_flag` / `query_leaf` as needed (still under
 *    range lock rules where applicable).
 * 5) `leaf_unbind_range` (or `leaf_unbind`): same range lock; INC walk validates
 *    every leaf is VALID, then the same walker flips to DEC (no second init) and
 *    unlinks rmap + clears VALID to LAZY without rewriting `vs_ptr`.
 * 6) `delete_range` when dropping reservation entirely (caller removes PTEs first
 ...
 * - DELETE: require deletable leaf, then `radix_node_clear` each crossed L3 slot.
 ...
 * ----------------------------------------------------
 * Single L3 walk with RADIX_TREE_DIRECTION_INC: per page validate LAZY + !VALID,
 * then `radix_leaf_link_rmap` + set VALID (no `vs_ptr` write). Any failure
 * (validation, `link_rmap`, or short walk vs `page_number`) jumps to rollback:
 * `while (page_index > 0 && walk)` is a no-op when nothing was bound. Rollback:
 * flip only `walk.direction` to DEC (same pattern as `radix_range_lock_acquire`
 * clean_prev), unlink + restore LAZY — no extra `radix_tree_level_walk_init`.
 *
 * `leaf_unbind_range`: validate with INC to the range end, then `direction = DEC`
 * and `do { … } while (remain && walk)` so PPN order matches VA order backward.
 *
 * Locking & SMP (I6)
 * ------------------
 * Any operation crossing multiple 2Mi bands takes each crossed L2 row’s bit-lock
 * in **ascending** 2MiB base order, then releases **descending**. Do not nest
 * `delete_range` while holding those locks. Single-path grow: L0 then L1 then L2
 * nested bit-locks (I3); updates to a row’s `value` use `radix_entry_update` with
 * the lock bit semantics (I4).
 *
 * Shared high-half (I5)
 * ---------------------
 * L0[256..511] may alias shared L1/L3. `delete_range` still clears only leaves
 * whose `vs_ptr` matches the caller; `leaf_bind_range` / `leaf_unbind_range` do not
 * compare `vs_ptr` to `vs` (insert already chose ownership). `change_*` follow
 * their per-API `vs_ptr` rules.
 *
 * Child occupancy & reclaim
 * -------------------------
 * Counts in L0/L1/L2 entries are **not** cross-vspace refcounts; they count used
 * slots in the next level. Phase 5 adjusts them with `radix_entry_update` + COUNT
 * delta when ranges cross 2Mi bands. Reclaim uses counts as guard for dropping
 * whole L3/L2/L1 pages (I7: structural pages grown on insert failure are not freed
 * until `destroy`, except shared L1 slab).
 *
 * rmap & PMM zone
 * ---------------
 * `radix_leaf_link_rmap` / `radix_leaf_unlink_rmap` run under `vs->pmm->zone` lock.
 * Typical lock order: vspace / radix band work first, then zone (see project
 * INVARIANTS / AI_CHECKLIST); do not invert while holding the same page in doubt.
 *
 * VA / parameters
 * ---------------
 ...
 * I3  Nested grow: L0→L1→L2 bit-lock order; no lock-order inversion vs other radix.
 * I4  Writes to a held row’s `value` go through `radix_entry_update` (+INHERIT_LOCK
 *     when the CAS lock must survive the store).
 * I5  Shared high-half: respect `vs_ptr` on leaves (see above).
 * I6  Multi-band: acquire all crossed L2 locks ascending 2Mi base, unlock descending.
 * I7  Table pages grown on failed insert are not freed until `destroy` (shared L1
 *     slab excepted).
 */
 ...
 * `vs_ptr` is whatever `insert_range` set (need not equal `vs`). Remap/overwrite
 * uses `change_*`.
 * Rollback on partial rmap failure restores lazy shadow from `leaf_flags` —
 * use the same `flags` vocabulary as the prior `insert_range` for this VA band.
 * No map()/unmap() here.
 */
...
/*
 * Restore radix shadow to LAZY for each page in the range (does not change `vs_ptr`).
 * `ppn_first` is the PPN of the first page; physical pages must be contiguous.
 * `page_number >= 1`. Does not adjust L2 occupancy; delete_range removes
...
/*
 * Update radix leaf flags after caller remaps PTE at `page_vaddr` from `old_ppn`
 * to `new_ppn`. When `old_ppn != new_ppn`, unlinks `rmap_list` from the old
 * `Page` and links into the new (same as leaf_unbind / leaf_bind under
 ...
 *   derived from the bootstrapped slab (`radix_entry_lock` / `radix_entry_update`
 *   per slot; never memcpy of another vspace’s entry words). Idempotent if
 *   already correct; -E_IN_PARAM if a slot already points elsewhere.
 * Install requires bootstrap to have completed.
 * After install, delete_range / leaf_bind / leaf_unbind / change_* on VAs in
 * that band use Radix_node_t::vs_ptr (set by insert_range) so shared L3 is
 * not corrupted by another vspace (see I5 and Range APIs above).
 * vmm_radix_tree_destroy does not free the shared high-half L1 band (L0[256..511]);
 * it only tears down L0[0..255] subtrees and the root page. Do not destroy radix
 * for one vspace while others still depend on the same shared L1 subtree unless
 * policy guarantees disjoint use (e.g. only root_vspace installs).
 */
error_t
