 ...
 * radix_l3_overlap_insert).
 * - @ref RADIX_RL_DELETE: each crossed L3 leaf: @c radix_node_clear if not
 *   @c radix_l3_undeletable (no @c PAGE_ENTRY_VALID — VALID leaves require
 * prior
 ...
 * - @ref RADIX_RL_QUERY_OR_CHANGE: does not bulk-clear L3 nodes; bind/unbind
 * and flag/query mutate leaves after acquire under the same held L2 set.
 *
 ...
 * Half-open intervals use @c [base, end) with @p end as the first byte after
 * the covered pages. Canonical-address policy is the caller's responsibility.
 ...
 * - I2: One L2 bit-lock covers that row's entry word, L3 array, leaf
 * flags/bind, and occupancy updates for that 2 MiB band.
...
/**
 * @brief One radix table entry word at L0, L1, or L2 (512 entries per 4KiB
 * page).
 */
typedef struct {
        u64 value; /*!< Packed lock, valid, count, and child pointer (see
                      @file). */
} Radix_entry_t;

/**
 * @brief L3 shadow node: one per 4KiB slot under an L2 entry (512 nodes / 16
 * KiB of L3 storage per L2 row).
 *
 * @par flags
...
/**
 * @brief Compute @p *vaddr_end_out = @p vaddr_start + @p page_number * @c
 * PAGE_SIZE and run the **only** radix VA-band policy check for that half-open
 * interval.
 *
 ...
 *
 * @return true on success, false if @p page_number is zero, @p vaddr_end_out is
 * NULL, the span overflows @c vaddr, or @c radix_check_range rejects the band.
 */
 ...
 * @param vaddr_start  Interval start (page-aligned; interval is [vaddr_start,
 * vaddr_end)).
 * @param vaddr_end    Interval end (first byte not in the interval; same
 * convention as internal @c radix_range_lock_acquire). Must match @p
 * *vaddr_end_out from a successful @ref vmm_radix_tree_calculate_end_check on
 * the same @p vaddr_start and page count (do not hand-roll @c vaddr_start + n *
 ...
 *                            supply @p vaddr_end from @ref
 * vmm_radix_tree_calculate_end_check; this function does not re-check radix
 * VA-band policy).
 *
 ...
 */
error_t vmm_radix_tree_lock_range_big(VSpace* vs, vaddr vaddr_start,
                                      vaddr vaddr_end);
 ...
 * @param vaddr_start Same @p vaddr_start passed to @ref
 * vmm_radix_tree_lock_range_big.
 * @param vaddr_end   Same @p vaddr_end passed to @ref
 * vmm_radix_tree_lock_range_big.
 *
 ...
 *      Takes @c radix_entry_lock on the chosen L2 row while reading that row
 * and scanning its L3 slots (see @c clone_vspace radix comment: L0 alone does
 *      not exclude concurrent L2 holders).
 *
 * @param direction @ref RADIX_TREE_DIRECTION_INC or @ref
 ...
 *                         with @c N >= 1.
 * @param flags_out      Optional; on success, written with the common
 *                         @c Radix_node_t::flags for every page in the run
 *                         (may be NULL to omit).
 *
 ...
 * @param flags       Initial shadow flags for each leaf (typically include
 * LAZY).
 * @param vaddr_end   Interval end for @c [ @p vaddr_start , @p vaddr_end );
 * from
 *                   @ref vmm_radix_tree_calculate_end_check(@p vaddr_start,
 * n_pages, &vaddr_end).
 *
...
/**
 * @brief Clear VALID and unlink rmap for each page in range; restore LAZY
 * shadow; does not change `owner`.
 *
 * @param vs           Target vspace.
...
/**
 * @brief Bind a single 4KiB leaf (implemented as @ref
 * vmm_radix_tree_leaf_bind_range on @c [ @p vaddr_start , @p vaddr_start +
 * PAGE_SIZE )).
 */
error_t vmm_radix_tree_leaf_bind(VSpace* vs, vaddr vaddr_start, ppn_t ppn,
                                 ENTRY_FLAGS_t leaf_flags);

/**
 * @brief Unbind a single 4KiB leaf (implemented as @ref
 *        vmm_radix_tree_leaf_unbind_range on @c [ @p vaddr_start , @p
 * vaddr_start + PAGE_SIZE )).
 */
error_t vmm_radix_tree_leaf_unbind(VSpace* vs, vaddr vaddr_start, ppn_t ppn);

/**
 * @brief After caller remaps PTE at @p vaddr_start from @p old_ppn to @p
 * new_ppn, update radix leaf wiring (rmap unlink/link under @c vs->pmm->zone
 *        when PPN changes).
 *
 * @param vs           Target vspace.
 * @param vaddr_start  User VA of the leaf.
 * @param vaddr_end    One-page band end from @ref
 * vmm_radix_tree_calculate_end_check(
 ...
 */
error_t vmm_radix_tree_change_leaf_ppn(VSpace* vs, vaddr vaddr_start,
 ...
 */
error_t vmm_radix_tree_change_leaf_ppn_flag(VSpace* vs, vaddr vaddr_start,
                                            vaddr vaddr_end, ppn_t old_ppn,
                                            ppn_t new_ppn,
 ...
 * vmm_radix_tree_unlock_range_small.
 */
error_t vmm_radix_tree_change_range_flag(VSpace* vs, vaddr vaddr_start,
                                         vaddr vaddr_end,
 ...
 * @param vs       Target vspace.
 *
 * @return Pointer to the L0 @ref Radix_entry_t array (root), or NULL on
 * failure.
 */
 ...
 * @param vs       Non-NULL vspace used for allocation context (see
 * implementation).
 *
 * @return @c REND_SUCCESS or an error code.
 *
