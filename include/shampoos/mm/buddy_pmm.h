...
static inline void frame_list_add_head(struct page_frame *bucket_head, struct page_frame *new_node,
                                       struct page_frame *head) {
    __frame_list_add(bucket_head, new_node, head, bucket_head + head->next);
}
static inline void frame_list_add_tail(struct page_frame *bucket_head, struct page_frame *new_node,
                                       struct page_frame *head) {
    __frame_list_add(bucket_head, new_node, bucket_head + head->prev, head);
}
static inline void __frame_list_del(struct page_frame *bucket_head, struct page_frame *prev,
...
static inline void frame_list_del_init(struct page_frame *bucket_head, struct page_frame *node) {
    __frame_list_del(bucket_head, bucket_head + node->prev, bucket_head + node->next);
    node->prev = node - bucket_head;
    node->next = node - bucket_head;
}
static inline void frame_list_del(struct page_frame *bucket_head, struct page_frame *node) {
    frame_list_del_init(bucket_head, node);
}
static inline bool frame_list_only_one(struct page_frame *bucket_head, struct page_frame *head) {
    struct page_frame *next;
    struct page_frame *prev;

    next = bucket_head + head->next;
    prev = bucket_head + head->prev;
