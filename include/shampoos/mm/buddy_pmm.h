...
static inline void	__frame_list_add(struct page_frame *bucket_head,
		struct page_frame *new_node, struct page_frame *prev,
	...
	static inline void frame_list_add_head(struct page_frame *bucket_head,
		struct page_frame *new_node, struct page_frame *head)
	{
	...
	{
		__frame_list_del(bucket_head, bucket_head + node->prev, bucket_head
			+ node->next);
