...
u64								entry_per_bucket[BUDDY_MAXORDER + 1],
...
static void	calculate_avaliable_phy_addr_end(void)
{
	...
	buddy_pmm.avaliable_phy_addr_end = ROUND_DOWN(buddy_pmm.avaliable_phy_addr_end,
			MIDDLE_PAGE_SIZE);
}
u64	calculate_pmm_space(void)
{
	...
	for (int order = 0; order <= BUDDY_MAXORDER; ++order)
	{
		...
		pages_per_bucket[order] = ROUND_UP(entry_per_bucket[order]
				* sizeof(struct page_frame), PAGE_SIZE) / PAGE_SIZE;
	}
	for (int order = 0; order <= BUDDY_MAXORDER; ++order)
		...
		paddr buddy_phy_start, paddr buddy_phy_end)
{
	...
	for (int i = 0; i < buddy_pmm.m_regions->region_count; i++)
	{
		...
		for (int order = 0; order <= BUDDY_MAXORDER; ++order)
		{
			...
			for (paddr addr_iter = sec_start_addr; addr_iter < sec_end_addr; addr_iter
				+= size_in_this_order)
			{
...
static void	pmm_init_zones(void)
{
	...
	for (int mem_zone = 0; mem_zone < ZONE_NR_MAX; ++mem_zone)
	{
		...
		for (int order = 0; order <= BUDDY_MAXORDER; ++order)
		{
			zone->zone_head_frame[order] = &(buddy_pmm.buckets[order].pages[IDX_FROM_PPN(order,
						PPN(zone->zone_lower_addr))]);
			zone->avaliable_frame[order] = NULL;
			zone->zone_total_pages = zone->zone_total_avaliable_pages = 0;
		}
		for (paddr addr_iter = 0; addr_iter < buddy_pmm.avaliable_phy_addr_end; addr_iter
			+= (PAGE_SIZE << BUDDY_MAXORDER))
		{
...
int	pmm_alloc_zone(int alloc_order, int zone_number)
{
	...
	while (tmp_order > alloc_order)
	{
		...
		child_order_avaliable_header = GET_AVALI_HEAD_PTR(zone_number, tmp_order
				- 1);
		...
		{
			GET_AVALI_HEAD_PTR(zone_number,
					tmp_order) = buddy_pmm.buckets[tmp_order].pages
				+ del_node->next;
...
static error_t	pmm_free_one(u32 ppn)
{
	...
	while (tmp_order <= BUDDY_MAXORDER)
	{
		...
		buddy_index = (buddy_index == index) ? (buddy_index
				+ 1) : (buddy_index);
			...
			|| tmp_order == BUDDY_MAXORDER)
		{
			...
			avaliable_header = GET_AVALI_HEAD_PTR(zone_number,
					tmp_order) = insert_node;
			break ;
		}
		/*else try merge*/
...
struct buddy					buddy_pmm = {.m_regions = &m_regions,
						.pmm_init = pmm_init, .pmm_alloc = pmm_alloc,
						.pmm_free = pmm_free};
