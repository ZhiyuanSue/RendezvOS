...
void nexus_print(struct nexus_node* nexus_root)
{
        ...
        pr_debug("=== [ NEXUS ] ===\n");
        pr_debug("[ MANAGE PAGES ]\n");
        if (nexus_root->backup_manage_page) {
                pr_debug("Backup Page: Yes 0x%x\n",
                      (vaddr)nexus_root->backup_manage_page);
        } else {
                pr_debug("Backup Page: No \n");
        }
        ...
        while (tmp_mp != manage_page_list_entry) {
                ...
                pr_debug("\t[ NEXUS ] have empty entry manage page %d with addr 0x%x: %d record can use\n",
                      manage_page_num,
                      (vaddr)manage_page,
        ...
        while (tmp_rb) {
                ...
                if (tmp_node)
                        pr_debug("\t[ NEXUS ] Used page: start vaddr 0x%x, size 0x%x\n",
                              tmp_node->start_addr,
                              tmp_node->size);
