...
struct multiboot_tag_framebuffer mtb2_fb_value;
// TODO: mtb2_fb_value
void *fb;
...
void fb1_map_pages()
{
        ...
        u64 fb_space_page_num = mtb1_fb_value.framebuffer_pitch
                                * mtb1_fb_value.framebuffer_height / PAGE_SIZE;
...
void fb1_show()
{
        ...
        case MULTIBOOT_FRAMEBUFFER_TYPE_INDEXED: {
                ...
                for (i = 0; i < mtb1_fb_value.framebuffer_palette_num_colors;
                     i++) {
        ...
        for (i = 0; i < mtb1_fb_value.framebuffer_width
                    && i < mtb1_fb_value.framebuffer_height;
             i++) {
                switch (mtb1_fb_value.framebuffer_bpp) {
                case 8: {
                        u8 *pixel =
                                fb + mtb1_fb_value.framebuffer_pitch * i + i;
                        *pixel = color;
                } break;
                case 15:
                case 16: {
                        u16 *pixel = fb + mtb1_fb_value.framebuffer_pitch * i
                                     + 2 * i;
                        *pixel = color;
                } break;
                case 24: {
                        u32 *pixel = fb + mtb1_fb_value.framebuffer_pitch * i
                                     + 3 * i;
...
void fb2_show()
{
        u32 color;
        unsigned i;
        void *fb = (void *)(unsigned long)mtb2_fb_value.common.framebuffer_addr;

        switch (mtb2_fb_value.common.framebuffer_type) {
        case MULTIBOOT_FRAMEBUFFER_TYPE_INDEXED: {
                unsigned best_distance, distance;
                struct multiboot_color *palette;

                palette = mtb2_fb_value.framebuffer_palette;

                color = 0;
                best_distance = 4 * 256 * 256;

                for (i = 0; i < mtb2_fb_value.framebuffer_palette_num_colors;
                     i++) {
                        distance = (0xff - palette[i].blue)
                                           * (0xff - palette[i].blue)
                                   + palette[i].red * palette[i].red
        ...
        case MULTIBOOT_FRAMEBUFFER_TYPE_RGB:
                color = ((1 << mtb2_fb_value.framebuffer_blue_mask_size) - 1)
                        << mtb2_fb_value.framebuffer_blue_field_position;
                break;
        ...
        for (i = 0; i < mtb2_fb_value.common.framebuffer_width
                    && i < mtb2_fb_value.common.framebuffer_height;
             i++) {
                switch (mtb2_fb_value.common.framebuffer_bpp) {
                case 8: {
                        u8 *pixel = fb
                                    + mtb2_fb_value.common.framebuffer_pitch * i
                                    + i;
                        *pixel = color;
                } break;
                case 15:
                case 16: {
                        u16 *pixel =
                                fb + mtb2_fb_value.common.framebuffer_pitch * i
                                + 2 * i;
                        *pixel = color;
                } break;
                case 24: {
