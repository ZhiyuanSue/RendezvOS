...
void	log_init(void *log_buffer_addr, int log_level)
{
	...
	{
		LOG_BUFFER.LOG_BUF[i].start_addr = log_buffer_addr + i
			* LOG_BUFFER_SINGLE_SIZE;
...
void	printk(const char *format, int log_level, ...)
{
	...
	if (log_level <= LOG_BUFFER.log_level)
	{
		...
		log_print(LOG_BUFFER.LOG_BUF[LOG_BUFFER.cur_buffer_idx].start_addr
			+ LOG_BUFFER.cur_buffer_offset, format, arg_list);
		va_end(arg_list);
	}
}
